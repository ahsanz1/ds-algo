 ...
 * achieve by buying & selling stock. You can buy & sell K times. You must have
 ...
 *
 * TC -> 2^n, SC -> O(n) -> ASS
 * TC DP -> O(N x 2 x K), SS -> O(n) + O(N x 2 x K) -> 2 states for buy value & K states for cap value
 */
...
const dpArr = new Array(n)
  .fill(null)
  .map(() => new Array(2).fill(null).map(() => new Array(k + 1).fill(-1)));
...
const dpTabulation = new Array(n + 1)
  .fill(null)
  .map(() => new Array(2).fill(null).map(() => new Array(k + 1).fill(0)));
...
const bestTimeToBuyAndSellStocks3Tabulation = () => {
  ...
  for (let buy = 0; buy <= 1; buy++)
    for (let cap = 0; cap <= k; cap++) dpTabulation[n][buy][cap] = 0;
...
console.log(bestTimeToBuyAndSellStocks3(0, 1, k));
console.log(bestTimeToBuyAndSellStocks3DP(0, 1, k));
