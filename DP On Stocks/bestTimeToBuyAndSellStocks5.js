 ...
 * bought stock before you sell it. You CANNOT BUY ON NEXT DAY ONCE YOU HAVE SOLD. BUY ON i + 1
 * NOT ALLOWED.
 *
...
const bestTimeToBuyAndSellStocks2 = (i, buy) => {
  /**
   * Because i + 2 can become out of bound in the case of i = n - 1
   */
  if (i >= n) return 0;
  ...
  if (buy) {
    ...
    const take = prices[i] + bestTimeToBuyAndSellStocks2(i + 2, 1);
...
const bestTimeToBuyAndSellStocks2DP = (i, buy) => {
  ...
  let profit = 0;
  if (buy) {
    const take = -prices[i] + bestTimeToBuyAndSellStocks2(i + 2, 0);
    const notTake = 0 + bestTimeToBuyAndSellStocks2(i + 1, 1);
    profit = Math.max(take, notTake);
    dpArr[i][buy] = profit;
  } else {
    const take = prices[i] + bestTimeToBuyAndSellStocks2(i + 1, 1);
    const notTake = 0 + bestTimeToBuyAndSellStocks2(i + 1, 0);
    profit = Math.max(take, notTake);
    dpArr[i][buy] = profit;
  }
...
//Declare DP as n+2
const dpTabulation = new Array(n + 2).fill(-1).map(() => new Array(2).fill(-1));
  ...
  for (let i = n - 1; i >= 0; i--) {
    for (let buy = 0; buy <= 1; buy++) {
      if (buy) {
        ...
        const take = prices[i] + dpTabulation[i + 2][1];
